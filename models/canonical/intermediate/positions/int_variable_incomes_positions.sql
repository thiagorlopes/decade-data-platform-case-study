...
WITH joined AS (
    ...
    SELECT
        ...
        bal.quantity,
        bal.closing_price
    FROM {{ latest_delivery(ref('stg_openfinance__variable_incomes_positions_detail')) }} AS det
    FULL JOIN {{ latest_delivery(ref('stg_openfinance__variable_incomes_positions_balances')) }} AS bal
