...
Optional: the exploratory notebooks under [`notebooks/`](notebooks/) sit outside the reproducible build. To read them without installing anything, open [the folder on GitHub](notebooks/), which renders each `.ipynb` inline. To re-run them, install their dependencies on the host and open the folder in Jupyter:
...
**`make build`** runs models + tests and writes `decade.duckdb`. Run it after every model edit, and first on a fresh clone.

**`make ui`** opens the [DuckDB UI](https://duckdb.org/docs/stable/core_extensions/ui.html) at http://localhost:4213: a schema browser, a notebook-style SQL editor, and result grids, all read-only. It runs as a background service, so it survives closed terminals and restarts itself after a crash. Stop it with `make ui-stop`. The UI attaches the warehouse read-only as the `decade` catalog. Queries use full paths (`decade.consumption.fct_holdings`), so they run in any notebook with no setup. The UI cannot open the warehouse as its main database in read-only mode: its result cache needs a writable session ([duckdb-ui#61](https://github.com/duckdb/duckdb-ui/issues/61)). DuckDB allows only one writer, so `make build`, `make run`, `make test`, and `make shell` pause the UI, take the write lock, and start the UI again. Refresh the browser tab after a build to reconnect.
...
Both targets fan out by layer: `staging`, `canonical`, `consumption`, plus `dbt_test__audit` for stored test failures. On Databricks the schema boundary is also the access boundary, so grants attach to schemas from the [Compliance](#compliance) section: consumers get `consumption` and nothing else. On DuckDB the same layout keeps one mental model across environments and stops the UI browser from flattening every layer into `main`. The catalog carries the same name on both engines: `decade`. Locally it is the `decade.duckdb` file. On Databricks it is a Unity Catalog catalog, created once with `CREATE CATALOG decade`. Consumer queries use the full path (`decade.consumption.fct_holdings`), so the same SQL text runs from `make ui`, `make shell`, `make consumers`, and the Databricks SQL editor, whatever the session's default catalog is.
...
The brief asks what was left out and why. Four entries.
